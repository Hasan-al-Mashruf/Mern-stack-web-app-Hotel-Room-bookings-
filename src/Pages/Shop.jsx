...
const Shop = () => {
    ...
    const { refetch, isLoading, error, data = [] } = useQuery({
        ...
        queryFn: () =>
            fetch(`https://hotelbookings-server.vercel.app/rooms?filterInfo=${filterInfo}&saleRooms=${saleRooms}`)
                .then((res) => res.json())
    })
                ...
                <div className='md:grid grid-cols-4 gap-6'>
                    <div className='col-span-3'>
                        <div className='flex items-center justify-between mb-5 flex-col md:flex-row'>
                            ...
                            <select className="select select-bordered w-full max-w-xs mt-5 md:mt-0" onChange={searchFilter}>
                                <option selected value='default'>Default search</option>
                                    ...
                                    <Link to={`/roomDetails/${room?._id}`}>
                                        <div className="card card-compact shadow-xl rounded-none">
                                            <div className='relative'>
                                                <div className='relative roomImage inline-block'>
                                                    ...
                                                    <div className='flex absolute bottom-0 bg-[#3d3d3d54] w-full pl-5 text-white'>
                                                        <div className='mr-5'>
                                                            <span className='people text-sm'>{room.people.adults} people</span>
                                                        </div>
                                                        <div>
                                                            <span className='children text-sm'>{room.people.children} children</span>
                                                        </div>
                                                    </div>
                                            ...
                                            <div className='flex justify-between bg-[#e0e0e052] items-center pl-4'>
                                                ...
                                                <div>
                                                    <button className='btn btn-primary rounded-none relative shopBtn ml-2 text-[12px] text-white'>Checkout Now</button>
                                                </div>
                                            </div>
                                        </div>
                                    </Link>
                    ...
                    <div>
                        <div className="relative overflow-hidden mt-10 md:mt-0 cursor-pointer" onClick={() => setSaleRooms('status')}>
                            <img src={Imaga2} alt="" />
                            <div className="content absolute z-10 left-1/2 top-1/2 -translate-y-1/2 -translate-x-1/2">
                                <h4 className='text-white md:text-3xl text-5xl text-center '>Huge sale</h4>
                            </div>
                            <div className="content absolute z-10 right-[-26px] top-4 rotate-45 w-28 ">
                                <h4 className='text-white text-md text-center bg-yellow-400'>40%</h4>
                            </div>
                            <div className='overlay absolute bg-black opacity-[.7]'>
                            </div>
                        </div>
                        <div className="relative overflow-hidden mt-10 cursor-pointer" onClick={() => searchFilter('rating')}>
                            <img src={Image3} alt="" />
                            <div className="content absolute z-10 left-1/2 top-1/2 -translate-y-1/2 -translate-x-1/2">
                                <h4 className='text-white md:text-3xl text-5xl text-center '>Top ratings</h4>
                            </div>
                            <div className="content absolute z-10 right-[-26px] top-4 rotate-45 w-28 ">
                                <h4 className='text-white text-md text-center bg-yellow-400'>5 star</h4>
                            </div>
                            <div className='overlay absolute bg-black opacity-[.7]'>
                            </div>
                        </div>
